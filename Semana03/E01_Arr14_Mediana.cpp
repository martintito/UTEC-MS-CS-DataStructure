...
int mediana(ListaArreglo<int> *lista, int n) {            
    //ordenamiento del arreglo
    ordenarLista(lista, n);      
    //mediana
    int med;
    // calculo de la mediana
    if(n % 2 != 0) { //numero de elementos es par
        lista->moverAPosicion(n/2);
    med = lista->getValor();
    } else { //numero de elementos es impar
    lista->moverAPosicion(n/2);
        med = (lista->getValorAnterior() + lista->getValor()) / 2;
    }            
    return med;
}
...
void casoMediana(){
    ...
    /*for(lista->moverAInicio(); //Inicializacion
        ...
        cout<<"Elemento: "<<lista->posicionActual()<<" "<<lista->getValor()<<endl;
    }*/
    
    cout << mediana(lista, n);    
    delete lista;
}
