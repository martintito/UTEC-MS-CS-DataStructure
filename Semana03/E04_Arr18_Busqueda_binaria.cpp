...
//Arr18 - Busqueda Binaria
bool busquedaBinaria(ListaArreglo<int> *lista, int n, int elemento) {
  ...
  lista->siguiente()){ //Incremento  
    ...
    lista->moverAPosicion(mitad);
    if(lista->getValor() == elemento){
      bandera = true;
      break;
    }
    if(lista->getValor() > elemento){
...
void casoBusquedaBinaria(){
    ...
    if (busquedaBinaria(lista, n, elemento)){
    cout << "SI" << endl;
    }
    else {
      cout << "NO" << endl;
